...
| Cloud SQL instance | `flagship-pg-<env>` |
| Memorystore Redis | `flagship-redis-<env>` |
| Admin token secret | `flagship-admin-token-<env>` |
...
ADMIN_TOKEN=$(gcloud secrets versions access latest --secret=flagship-admin-token-staging)
...
```bash
# 1. Add a new version
openssl rand -base64 32 | gcloud secrets versions add flagship-admin-token-<env> --data-file=-
...
gcloud secrets versions list flagship-admin-token-<env>
gcloud secrets versions destroy <old-version-N> --secret=flagship-admin-token-<env>
```
...
Response: check `gcloud redis instances describe flagship-redis-<env> --region $REGION`
...
gcloud sql instances patch flagship-pg-<env> --activation-policy=NEVER
...
gcloud redis instances delete flagship-redis-<env> --region "$REGION"
```
